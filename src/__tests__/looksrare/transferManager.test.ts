...
describe("Transfer manager", () => {
  ...
  it("transfer items from a single collection", async () => {
    ...
    await (await lrUser1.approveAllCollectionItems(addresses.MOCK_COLLECTION_ERC721)).wait();
    await (await lrUser1.grantTransferManagerApproval().call()).wait();
    const collectionERC721 = new Contract(addresses.MOCK_COLLECTION_ERC721, abiIERC721).connect(
      ethers.provider
    ) as ERC721;
    ...
    const items: BatchTransferItem[] = [
      {
        collection: addresses.MOCK_COLLECTION_ERC721,
  ...
  it("transfer items from multiple collections", async () => {
    const { addresses } = mocks;
    await (await lrUser1.approveAllCollectionItems(addresses.MOCK_COLLECTION_ERC721)).wait();
    await (await lrUser1.approveAllCollectionItems(addresses.MOCK_COLLECTION_ERC1155)).wait();
    (await lrUser1.grantTransferManagerApproval().call()).wait();
    const collectionERC721 = new Contract(addresses.MOCK_COLLECTION_ERC721, abiIERC721).connect(
      ethers.provider
    ) as ERC721;
    const collectionERC1155 = new Contract(addresses.MOCK_COLLECTION_ERC1155, abiIERC1155).connect(
      ethers.provider
    ) as ERC1155;
    ...
    const items: BatchTransferItem[] = [
      ...
      {
        collection: addresses.MOCK_COLLECTION_ERC1155,
    ...
    expect(newBalance).to.be.equal(10n);
  });
});
